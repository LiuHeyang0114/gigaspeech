...
Options:
    ...
    --time           # Length of speech(options: /_50/_100)
    # Data preparation related
...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        ...
        if [ "${num_splits_asr}" -gt 1 ]; then
            ...
            _opts+="--train_data_path_and_name_and_type kaldi_format_split/wav_${time}.scp,speech,${_type} "
            _opts+="--train_data_path_and_name_and_type kaldi_format_split/text_${time},text,text "
            _opts+="--train_shape_file kaldi_format_split/speech_shape_${time} "
            _opts+="--train_shape_file kaldi_format_split/text_shape_${time}.${token_type} "
            _opts+="--multiple_iterator true "
