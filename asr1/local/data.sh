...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    pushd GigaSpeech
    ./toolkits/kaldi/gigaspeech_data_prep.sh --train-subset S ${GIGASPEECH} ${abs_data_dir}
    pushd ${abs_data_dir}
    ln -s gigaspeech_train_s train
    ln -s gigaspeech_dev dev
    ln -s gigaspeech_test test
    popd
    popd
fi
